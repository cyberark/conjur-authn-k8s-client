...
func (tc TraceContext) Inject(ctx context.Context, carrier TextMapCarrier) {
	fmt.Printf("***TEMP*** Call to Inject()\n")
	...
	if ts := sc.TraceState().String(); ts != "" {
		fmt.Printf("***TEMP*** About to call carrier.Set() with trace state: %s\n", ts)
	...
	h := fmt.Sprintf("%.2x-%s-%s-%s",
		...
		sc.SpanID(),
		flags)
	fmt.Printf("***TEMP*** About to call carrier.Set() with trace parent: %s\n", h)
