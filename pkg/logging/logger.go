...
/*
	...
	the error and not append it to the new error message, as follows:
		...
		if err != nil {
			return nil, log.PrintAndReturnError("failed to run someMethod")
		}
	...
	to the error log. So we'll have the following code:
		...
		if err != nil {
			return nil, log.PrintAndReturnError(fmt.Sprintf("failed to run someMethod. Reason: %s", err))
		}
*/
...
const CAKC069E string = "CAKC069E Failed to create conjur secrets retriever"
