...
func TestGatherSettings(t *testing.T) {
	TestCases := []struct {
		...
		expected    authenticator.AuthnSettings
	}{
		{
	...
	for _, tc := range TestCases {
		t.Run(tc.description, func(t *testing.T) {
			resultMap := authenticator.GatherSettings(&k8s.Config{}, tc.annotFunc, fromEnv)
...
func TestFromEnv(t *testing.T) {
	...
	for _, tc := range TestCases {
		t.Run(tc.description, func(t *testing.T) {
			// SETUP & EXERCISE
			setEnv(tc.env)
			config, err := authenticator.ConfigFromEnv(successfulMockReadFile)
